...
public class TokenInterceptor implements ContainerRequestFilter, ContainerResponseFilter, ReaderInterceptor, WriterInterceptor {
	 ...
	 * @return
	 */
	protected boolean checkHead(MultivaluedMap<String, String> headerMap, UriInfo uriInfo) {
		...
		if (errorCode.getErrCode().equals("0000")) {
			...
			logger.info("systemId={},token={},url={}, check failed", systemId, token, uriPath);
			Map<String, Object> retEntity = new HashMap<String, Object>();
			retEntity.put("message", errorCode.getErrDesc());
			Headers<Object> retHeaders = new Headers<Object>();
			retHeaders.add("Status", "401");
			ServerResponse serverResponse = new ServerResponse(retEntity,
					javax.ws.rs.core.Response.Status.OK.getStatusCode(), retHeaders);
			throw new WebApplicationException("systemId或token检查未通过", serverResponse);
		}
	}
	...
	private TokenService initTokenService() {
		...
		Assert.notNull(context, "SpringContainer初始化未完成");
		...
		reference.setProtocol("rmi"); //引用rmi协议
